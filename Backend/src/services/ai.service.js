...
`
    ...
    try {
        ...
        if (parsed.technicalQuestions && typeof parsed.technicalQuestions[0] === "string") {
            parsed.technicalQuestions = parsed.technicalQuestions.map(q => ({
                question: q,
                intention: "Assess technical knowledge",
                answer: "Explain concepts clearly with examples"
            }))
        }

        if (parsed.behavioralQuestions && typeof parsed.behavioralQuestions[0] === "string") {
            parsed.behavioralQuestions = parsed.behavioralQuestions.map(q => ({
                question: q,
                intention: "Evaluate behavioral skills",
                answer: "Use the STAR method (Situation, Task, Action, Result)"
            }))
        }

        if (parsed.skillGaps && typeof parsed.skillGaps[0] === "string") {
        ...
        if (parsed.preparationPlan && typeof parsed.preparationPlan[0] === "string") {
